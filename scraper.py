    ...
    for kw in keywords:
        try:
            ...
            if try_click_js_first(driver, btn):
                print(f"[CLICK] Button matched: {kw}", flush=True)
                return True
        except Exception:
            continue
    print(f"[WARN] No matching button found for keywords: {keywords}", flush=True)
    return False
        ...
        while current <= end_dt:
            ...
            if not date_already_recorded(PRICE_FILE, date_str):
                rows_added = scrape_one_date(
                    ...
                    ["मूल्य", "Price", "Check", "डाटा जाँच्नुहोस्", "Price Check"]
                )
            ...
            if not date_already_recorded(ARRIVAL_FILE, date_str):
                rows_added = scrape_one_date(
                    ...
                    ["आगमन डाटा जाँच्नुहोस्", "आगमन", "Arrival", "Check", "डाटा जाँच्नुहोस्", "Show", "Submit"]
                )
                if rows_added == 0:
                    print(f"[WARN] ⚠️ Arrival data not found or table empty for {date_str}", flush=True)
                else:
                    print(f"[OK] ✅ Arrival data written for {date_str}", flush=True)
